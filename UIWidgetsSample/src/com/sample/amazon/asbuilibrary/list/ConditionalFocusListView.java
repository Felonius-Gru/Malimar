...
public class ConditionalFocusListView extends ListView {
	...
	public ConditionalFocusListView(Context context, AttributeSet attrs,
		...
		TypedArray typedAttrs = context.obtainStyledAttributes(attrs,
				R.styleable.ConditionalFocusListView);
		mSelectedPositionFromTopPX = typedAttrs
				.getDimensionPixelOffset(
						R.styleable.ConditionalFocusListView_selected_position_from_top,
						mSelectedPositionFromTopPX);
		typedAttrs.recycle();
	}
	...
	/**
	 * Forces a re-selection of the current item. This can be useful if the
	 * selected item changed its layout and we need to make ListView aware of
	 * this change.
	 */
	public void recalculateCurrentPosition() {
		if (getCount() > 0) {
			setSelectionFromTop(getSelectedItemPosition(),
					mSelectedPositionFromTopPX);
		}
	}
	 ...
	 * 
	 * @param selectedPositionFromTopPX
	 *            The desired offset
	 */
