...
public class PlaybackOverlayFragment extends android.support.v17.leanback.app.PlaybackOverlayFragment {
    ...
    private static final boolean SHOW_IMAGE = false;//PRIMARY_CONTROLS <= 5;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mSelectedMovie = (Movie) getActivity()
                .getIntent().getSerializableExtra(Content.MOVIE);

        if (savedInstanceState != null) {

        }

        List<Category> moviesCategories = VideoProvider.getCurrentMovieList();

        for (Category category : moviesCategories) {
            if (mSelectedMovie.getCategory().equalsIgnoreCase(category.getTitle())) {
                mItems.addAll(category.getMovieList());

                int j = 0;

                for (Movie movie : category.getMovieList()) {
                    if (mSelectedMovie.getId().equalsIgnoreCase(movie.getId())) {
                        mCurrentItem = j;
                    }
    ...
    private int getDuration() {
        ...
        mDuration = 30;
        return safeLongToInt(mDuration);
    }
...
//        mSecondaryActionsAdapter.add(new PlaybackControlsRow.ClosedCaptioningAction(sContext));
    ...
    public void startProgressAutomation() {
        mRunnable = new Runnable() {
            ...
            public void run() {
                ...
                int currentTime = mCallback.videoPlayed(); //mPlaybackControlsRow.getCurrentTime() + updatePeriod;
    ...
    static class DescriptionPresenter extends AbstractDetailsDescriptionPresenter {
        ...
        protected void onBindDescription(ViewHolder viewHolder, Object item) {
            ...
            viewHolder.getSubtitle().setText("Fight Network");
            //((Movie) item).getStudio());
        }
    }
